<template>
  <ConfigSettingPage title="Nguồn khách">
    <div class="nguon-khach page-list">
      <CustomCard shadow="hover" class="filter-card">
        <CustomRow :gutter="12" class="toolbar">
          <CustomCol :xs="12" :sm="12" :md="8" :lg="8">
            <CustomInput
              v-model="keyword"
              placeholder="Tìm theo tên nguồn khách, ghi chú..."
              clearable
              style="width: 100%"
              @clear="onSearch"
      ...
      <CustomCard shadow="hover" class="table-card">
        <template #header>
          <div class="card-header">
            <span class="card-title">Danh sách nguồn khách</span>
        ...
        >
          ...
          <CustomTableColumn
            v-if="columnSettings.isColumnVisible('ten_nguon_khach')"
            prop="ten_nguon_khach"
      ...
      <CustomDialog
        ...
        :title="editingId ? 'Sửa nguồn khách' : 'Thêm nguồn khách'"
        :width="640"
      >
        <CustomForm ref="formRef" :model="form" :rules="rules">
          <CustomRow :gutter="16">
            <CustomCol :xs="24" :sm="12">
              <CustomFormItem label="Tên nguồn khách" prop="ten_nguon_khach">
                <CustomInput v-model="form.ten_nguon_khach" placeholder="VD: Facebook Ads" />
              </CustomFormItem>
            </CustomCol>
            <CustomCol :xs="24" :sm="12">
...
import {
  createDanhMucNguonKhach,
  deleteDanhMucNguonKhach,
  fetchDanhMucNguonKhach,
  updateDanhMucNguonKhach,
} from '@/api/danhMucNguonKhach'
...
const tableColumns = [
  { key: 'ten_nguon_khach', label: 'Tên nguồn khách' },
  { key: 'ghi_chu', label: 'Ghi chú' },
  { key: 'trang_thai', label: 'Trạng thái' },
]
const columnSettings = useTableColumns('he-thong.nguon-khach', tableColumns)
  ...
  return [
    {
      ...
      tooltip: activeCount
        ? `Bật ${activeCount} nguồn khách đang tắt`
        : 'Chọn nguồn khách không hoạt động để bật',
    },
    {
      key: 'deactivate',
      label: 'Tắt',
      ...
      tooltip: inactiveCount
        ? `Tắt ${inactiveCount} nguồn khách đang hoạt động`
        : 'Chọn nguồn khách đang hoạt động để tắt',
    },
    {
      key: 'delete',
      label: 'Xóa',
      ...
      tooltip: selectedCount.value
        ? `Xóa ${selectedCount.value} nguồn khách đã chọn`
        : 'Chọn nguồn khách để xóa',
    },
  ]
})
...
const rules = {
  ten_nguon_khach: [{ required: true, message: 'Vui lòng nhập tên nguồn khách', trigger: 'blur' }],
...
async function toggleStatus(row) {
  ...
  try {
    await updateDanhMucNguonKhach(row.id, {
      ten_nguon_khach: row.ten_nguon_khach,
      ghi_chu: row.ghi_chu || null,
      trang_thai: value,
    })
    row.trang_thai = value
    ElMessage.success(
      value === ACTIVE ? 'Đã bật nguồn khách.' : 'Đã tắt nguồn khách.',
    )
    return true
  } catch {
    return false
...
async function bulkSetStatus(target) {
  ...
  await ElMessageBox.confirm(`${label} ${ids.length} nguồn khách đã chọn?`, 'Xác nhận', {
    type: 'warning',
  ...
  try {
    ...
    await runBulk(ids, async (id) => {
      ...
      await updateDanhMucNguonKhach(id, {
        ten_nguon_khach: row?.ten_nguon_khach,
        ghi_chu: row?.ghi_chu || null,
        trang_thai: target,
      })
    })
    ElMessage.success(`Đã ${label.toLowerCase()} ${ids.length} nguồn khách.`)
    await loadItems()
  } catch {
    // interceptor
  } finally {
...
async function bulkRemove() {
  ...
  await ElMessageBox.confirm(`Xóa ${ids.length} nguồn khách đã chọn?`, 'Xác nhận', {
    type: 'warning',
  ...
  try {
    await runBulk(ids, (id) => deleteDanhMucNguonKhach(id))
    ElMessage.success(`Đã xóa ${ids.length} nguồn khách.`)
    await loadItems()
  } catch {
    // interceptor
  } finally {
...
async function save() {
  ...
  saving.value = true
  const payload = {
    ten_nguon_khach: form.ten_nguon_khach.trim(),
  ...
  try {
    if (editingId.value) {
      await updateDanhMucNguonKhach(editingId.value, payload)
      ElMessage.success('Đã cập nhật nguồn khách.')
    } else {
      await createDanhMucNguonKhach(payload)
      ElMessage.success('Đã thêm nguồn khách.')
    }
...
async function remove(row) {
  await ElMessageBox.confirm(`Xóa nguồn khách "${row.ten_nguon_khach}"?`, 'Xác nhận', {
    type: 'warning',
    confirmButtonText: 'Xóa',
    cancelButtonText: 'Hủy',
  })
  ...
  try {
    await deleteDanhMucNguonKhach(row.id)
    ElMessage.success('Đã xóa nguồn khách.')
    await loadItems()
  } catch {
    // Lỗi đã được axios interceptor xử lý
  }
