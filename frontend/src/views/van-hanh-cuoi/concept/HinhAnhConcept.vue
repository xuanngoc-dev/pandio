<template>
  <div class="hinh-anh-concept page-list">
    ...
    <CustomCard shadow="hover" class="gallery-card">
      <template #header>
        <div class="card-header">
          <div class="card-header-left">
            ...
            <span class="card-title">Hình ảnh concept</span>
          </div>
      ...
      <div v-loading="loading" class="gallery-wrap">
        <el-empty
          ...
          description="Chưa có hình ảnh trong thư mục concept."
        />
    ...
    <CustomDialog v-model="dialogVisible" title="Sửa tên hình ảnh" :width="480">
      ...
      <CustomForm ref="formRef" :model="form" :rules="rules">
        <CustomFormItem label="Tên file" prop="name">
          <CustomInput
            v-model="form.name"
            placeholder="vd: concept-studio.jpg"
            maxlength="255"
          />
        </CustomFormItem>
      </CustomForm>
...
import {
  completeHinhAnhConceptUpload,
  deleteHinhAnhConcept,
  fetchHinhAnhConcept,
  giaiNenHinhAnhConcept,
  updateHinhAnhConcept,
  uploadHinhAnhConceptChunk,
} from '@/api/concept'
import {
  CustomButton,
  CustomCard,
...
import { useHinhAnhFolderUpload } from '@/composables/useHinhAnhTrangPhucUpload'
import { apiOrigin, mediaUrl } from '@/utils/media'

const VIEW_GRID = 'grid'
const VIEW_TABLE = 'table'
const VIEW_MODE_KEY = 'hinh-anh-concept.viewMode'
...
const {
  ...
  uploadSelection,
  extractExisting,
} = useHinhAnhFolderUpload({
  uploadChunk: uploadHinhAnhConceptChunk,
  completeUpload: completeHinhAnhConceptUpload,
  extractZip: giaiNenHinhAnhConcept,
})
...
function validateFileName(_rule, value, callback) {
  ...
  if (!ext || name.lastIndexOf('.') <= 0) {
    callback(new Error('Tên file phải gồm tên và đuôi (vd: concept-studio.jpg).'))
    return
  }
...
async function loadItems() {
  ...
  clearSelection()
  try {
    const { data } = await fetchHinhAnhConcept({
      page: page.value,
...
async function deleteFiles(paths, confirmText) {
  ...
  try {
    const { data } = await deleteHinhAnhConcept({ paths })
